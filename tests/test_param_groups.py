...
def test_assign_group_at_creation():
    """Groups assigned at Param creation."""
    p = Param("p", 1.0, group=2)
    assert p.group == 2


def test_assign_group_later():
...
def test_reassign_group_updates_graph():
...
def test_param_order_by_group(grouped_model):
...
def test_groups_at_top_level(grouped_model):
...
def test_groups_within_hierarchy():
...
def test_groups_mixed_hierarchy():
...
def test_get_values_multi_group(grouped_model, scheme):
...
@pytest.mark.parametrize("scheme", ["array", "list", "dict"])
def test_get_values_single_group(scheme):
...
def test_get_values_specific_group(grouped_model):
...
def test_set_values_multi_group_array(grouped_model):
...
def test_set_values_multi_group_list(grouped_model):
...
def test_set_values_multi_group_dict(grouped_model):
...
def test_round_trip_values_multi_group(grouped_model):
...
def test_batch_dims_independent_per_group(grouped_model):
...
def test_batch_dims_both_groups_different_sizes(grouped_model):
...
def test_batch_dims_multi_dim_batch(grouped_model):
...
def test_find_param_with_group(grouped_model):
...
def test_find_index_multi_group(grouped_model):
...
def test_find_index_different_groups(grouped_model):
...
def test_find_index_list_scheme(grouped_model):
...
def test_find_param_out_of_range(grouped_model):
    """find_param raises IndexError for out-of-range index."""
    with pytest.raises(IndexError):
        grouped_model.find_param(100, group=0)


def test_find_index_unknown_param(grouped_model):
...
def test_to_valid_multi_group(bounded_grouped_model):
...
def test_from_valid_multi_group(bounded_grouped_model):
...
@pytest.mark.parametrize("scheme", ["array", "list", "dict"])
def test_valid_context_multi_group(bounded_grouped_model, scheme):
...
def test_to_valid_with_group_arg(bounded_grouped_model):
...
def test_forward_with_params_kwarg(grouped_model):
...
def test_forward_consistency(grouped_model):
...
def test_active_context_multi_group(grouped_model):
...
def test_set_values_blocked_in_active_context(grouped_model):
...
def test_nested_active_context_multi_group(grouped_model):
...
def test_active_cache_with_multi_group():
...
def test_groups_with_node_list():
...
def test_groups_with_node_tuple():
...
def test_collection_dynamic_param_groups():
...
def test_static_params_not_in_groups(grouped_model):
...
def test_to_dynamic_restores_groups(grouped_model):
...
def test_to_static_all_then_dynamic():
...
def test_get_values_after_all_static(grouped_model):
...
def test_override_in_multi_group():
...
def test_many_groups():
    ...
    class ManyParams(Module):
        def __init__(self, n):
            super().__init__()
            for i in range(n):
                setattr(self, f"p{i}", Param(f"p{i}", float(i), shape=(), dynamic=True, group=i))
...
def test_non_contiguous_groups():
...
def test_reassign_all_to_same_group(grouped_model):
...
def test_empty_group_after_static(grouped_model):
...
def test_single_param_per_group():
...
def test_forward_no_params_multi_group(grouped_model):
...
def test_group_with_none_value_param_raises():
...
def test_valid_context_forward_multi_group():
...
def test_valid_context_preserves_state(grouped_model):
...
def test_str_multi_group(grouped_model):
...
def test_param_order_str(grouped_model):
